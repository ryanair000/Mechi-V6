...
create or replace function private.mechi_prepare_competitive_profile()
returns trigger
language plpgsql
set search_path = ''
as $$
...
begin
  ...
  if v_default_rating is null then
    raise exception using errcode = '23514', message = 'competitive_game_not_available';
  end if;

  new.rating := v_default_rating;
  new.rank_tier := 'unranked';
  new.rank_division := null;
  new.placements_completed := 0;
...
revoke all on function private.mechi_prepare_competitive_profile() from public, anon, authenticated;

create trigger mechi_player_game_profiles_prepare
before insert on public.mechi_player_game_profiles
for each row execute function private.mechi_prepare_competitive_profile();
...
grant insert (profile_id, game_id) on table public.mechi_player_game_profiles to authenticated;
...
create policy mechi_player_game_profiles_owner_activate
on public.mechi_player_game_profiles
...
with check (
  ...
  and exists (
    ...
    from public.mechi_competitive_game_configs c
    join public.mechi_games g on g.id = c.game_id
    where c.game_id = mechi_player_game_profiles.game_id
      and c.competition_enabled = true
      and g.competition_enabled = true
      and g.is_active = true
  )
...
comment on table public.mechi_player_game_profiles is 'Game-specific competitive identity and aggregate record. Clients may activate only their own allowed game profile; competitive stats remain non-writable to client roles.';
